...
async def create_book(book : BookCreate, conn = Depends(get_conn)):
    try:
...
async def delete_book(id : UUID = Path(...,description="ID of the book to be deleted"), conn = Depends(get_conn)):
